...
public class InventoryController {
    ...
    private void showProductDialog(Product existingProduct) {
        ...
        Label catLbl = new Label("CATEGORY"); catLbl.setStyle("-fx-text-fill: -kmtm-text-dim;");
        Label nameLbl = new Label("PRODUCT NAME"); nameLbl.setStyle("-fx-text-fill: -kmtm-text-dim;");
        Label priceLbl = new Label("PRICE (₱)"); priceLbl.setStyle("-fx-text-fill: -kmtm-text-dim;");
        Label stockLbl = new Label("STOCK QUANTITY"); stockLbl.setStyle("-fx-text-fill: -kmtm-text-dim;");
    ...
    private void applyThemeToDialog(DialogPane dialogPane) {
        if (inventoryTable != null && inventoryTable.getScene() != null) {
            // Grab the exact stylesheets actively running on the main window
            javafx.collections.ObservableList<String> activeStyles = inventoryTable.getScene().getStylesheets();

            // Strip old styles and force the active theme directly onto the pane
            dialogPane.getStylesheets().setAll(activeStyles);
            dialogPane.getStyleClass().setAll("custom-dialog", "root");

            // Set a trap for the hidden Scene generated by the Dialog
