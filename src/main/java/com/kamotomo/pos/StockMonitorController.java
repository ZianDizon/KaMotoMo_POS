...
public class StockMonitorController {
    ...
    // --- THE BULLETPROOF SCENE INJECTION ---
    private void applyThemeToDialog(DialogPane dialogPane) {
        if (stockTable != null && stockTable.getScene() != null) {
            // 1. Grab the exact stylesheets actively running on the main window
            ObservableList<String> activeStyles = stockTable.getScene().getStylesheets();

            // 2. Strip old styles and force the active theme directly onto the pane
            dialogPane.getStylesheets().setAll(activeStyles);
            dialogPane.getStyleClass().setAll("custom-dialog", "root");

            // 3. Set a trap for the hidden Scene generated by the Dialog
